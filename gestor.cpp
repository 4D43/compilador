...
char ruta[100] = "C:/Users/S0/PROYECTOS/compilad/compilador";
char NArchivo[100] = "";
...
void seleccionar()
{
    ...
    if (usarJoin)
    {
        ...
        if (num_columnas_seleccionadas == -1)
        { // SELECT *
            num_columnas_seleccionadas = 0;
    ...
    if (usar_condicion_join)
    {
        ...
        { // Si no se encuentra y no está calificado, error
...
void ingresar()
{
                ...
                if (usar_condicion_join)
                {
                    ...
                    if (idx_col1 != -1 && idx_col2 != -1)
                    {
                        ...
                        if (strcmp(tipo_actual_str, "int") == 0)
                        {
                            ...
                            {
                                if (!(convertirAEntero(val1) == convertirAEntero(val2)))
                                    cumple_join = false;
                            }
                            else
                            {
                ...
                if (cumple_join)
                {
                            ...
                            if (valor_celda_where != nullptr)
                            {
                                ...
                                else if (strcmp(tipo_actual_str, "float") == 0)
                                {
                                    if (esFlotante(valor_celda_where) && esFlotante(valor_cond))
                                    {
                                        ...
                                        if (strcmp(operador, "=") == 0)
                                        {
                                            if (!(val_celda_float == val_cond_float))
                                                cumple_condiciones_where = false;
                                        }
                                        else if (strcmp(operador, "<") == 0)
                                        {
                                ...
                                else if (strcmp(tipo_actual_str, "str") == 0 || strcmp(tipo_actual_str, "string") == 0)
                                {
                                    ...
                                    if (strcmp(operador, "=") == 0)
                                    {
                                        if (!(strcmp(valor_celda_where, temp_valor_cond) == 0))
                                            cumple_condiciones_where = false;
                                    }
                                    else if (strcmp(operador, "!=") == 0 || strcmp(operador, "<>") == 0)
                                    {
                                    ...
                                    {
                                        std::cerr << "Advertencia: Operador '" << operador << "' no soportado para tipo STRING en WHERE.\n";
                                        cumple_condiciones_where = false;
                                    }
                                }
                                else
                    ...
                    if (cumple_condiciones_where)
                    {
                        ...
                        for (int i = 0; i < num_columnas_seleccionadas; ++i)
                        {
                            ...
                            if (dot_pos != NULL)
                            { // Columna calificada (ej. clientes.nombre)
                            ...
                            { // Columna no calificada, buscar en ambas tablas (preferencia a la izquierda)
                                for (int k = 0; k < num_campos_izq; ++k)
                                {
                                    if (strcmp(temp_col_name, columnas_registro[k]) == 0)
                                    {
                                ...
                                { // No encontrada en izquierda, buscar en derecha
                                    for (int k = 0; k < num_campos_der; ++k)
                                    {
                                        if (strcmp(temp_col_name, columnas_registro_join[k]) == 0)
                                        {
                ...
                for (int c = 0; c < numCondiciones && cumple_condiciones; ++c)
                {
                    ...
                    if (idx_col != -1 && idx_col < num_campos)
                    {
                        ...
                        else if (strcmp(tipo_columna_str, "str") == 0 || strcmp(tipo_columna_str, "string") == 0)
                        {
                            ...
                            if (strcmp(operador, "=") == 0)
                            {
                                if (!(strcmp(valor_celda, temp_valor_cond) == 0))
                                    cumple_condiciones = false;
                            }
                            else if (strcmp(operador, "!=") == 0 || strcmp(operador, "<>") == 0)
                            {
                            ...
                            {
                                std::cerr << "Advertencia: Operador '" << operador << "' no soportado para tipo STRING.\n";
                                cumple_condiciones = false;
                            }
                        }
                        else
    ...
    for (int i = 0; i < num_columnas_seleccionadas; ++i)
    {
        ...
        char *dot_pos = strchr(columnas_seleccionadas[i], '.');
        if (dot_pos != NULL)
        { // Si la columna está calificada (ej. tabla.columna)
        ...
        for (int j = 0; j < num_columnas_registro; ++j)
        {
            ...
            {
                if (dot_pos == NULL || strcmp(table_prefix, NArchivo) == 0)
                { // Si no está calificada o es de la tabla izquierda
        ...
        if (!tipo_encontrado && usarJoin)
        {
            for (int j = 0; j < num_columnas_registro_join; ++j)
            {
                ...
                {
                    if (dot_pos == NULL || strcmp(table_prefix, tabla_join) == 0)
                    { // Si no está calificada o es de la tabla derecha
